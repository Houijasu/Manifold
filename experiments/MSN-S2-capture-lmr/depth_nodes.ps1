<#
.SYNOPSIS
    Fixed-depth node counts with UseCaptureLMR on vs off, over a tactical FEN set.
...
param(
    ...
    [int[]]$Depths = @(10, 12, 14),
    [string]$OutFile = "experiments\MSN-S2-capture-lmr\depth-nodes.txt"
)
...
function Invoke-Search {
    ...
    $w.WriteLine("setoption name UseCaptureLMR value $($Enabled.ToString().ToLower())")
...
$lines = @()
$lines += "Capture LMR fixed-depth node counts -- $(Get-Date -Format s)"
